...
def test_make_random_dataset_one_battery():
    env = make(
        "battery",
        n_batteries=1,
        dataset={"name": "random-dataset", "n": 10000, "n_features": 3},
    )
        ...
        data.append(
            {
                "features": np.random.random(
                    (episode_len, sequence_length, n_features)
                ),
                "mask": np.random.random(
                    (episode_len, sequence_length, sequence_length)
                ),
                "prices": np.random.random((episode_len, 1)),
            }
        )

    ds = NEMDatasetAttention(n_batteries=2, train_episodes=data, test_episodes=data)
...
def test_attention_dataset_load_arrays():
    ...
    dss = [
        {
            "features": np.random.random(
                (episode_len, n_batteries, sequence_length, n_features)
            ),
            "mask": np.random.randint(
                0, 1, episode_len * n_batteries * sequence_length * sequence_length
            ).reshape(episode_len, n_batteries, sequence_length, sequence_length),
