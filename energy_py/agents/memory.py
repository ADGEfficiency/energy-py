...
class Memory(Utils):
    ...
    def add_experience(self, 
        ...
        args
            ...
            episode
        """
        logger.debug('adding experience for ep {} step {}'.format(episode, step))
    ...
    def get_random_batch(self, batch_size, save_batch=False):
        ...
        logger.debug('getting batch of size {} from memory'.format(sample_size))
        ...
        obs = np.array(obs, dtype=np.float32).reshape(sample_size, 
                                                      obs_space_dim)

        actions = np.array(acts, dtype=np.float32).reshape(sample_size,
                                                          act_space_dim)

        rewards = np.array(rews, dtype=np.float32).reshape(sample_size, 1)

        next_obs = np.array(next_obs, dtype=np.float32).reshape(sample_size,
                                                                obs_space_dim)

        term = np.array(term, dtype=np.bool_).reshape(sample_size, 1)
