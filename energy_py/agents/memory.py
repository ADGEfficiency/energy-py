...
class AgentMemory(Utils):
    ...
    def calculate_returns(self, episode_number, normalize_return=False):
        """
        ...
        if normalize_return == 'scale_only':
            rtns = rtns / rtns.std()
        if normalize_return == 'mean_scale':
            rtns = (rtns - rtns.mean()) / (rtns.std())
        if normalize_return == 'min_max':
    ...
    def get_episode_batch(self, episode_number, scaled_actions):
        ...
        returns = np.array(returns).reshape(-1, len(self.observation_space))
    ...
    def get_random_batch(self, batch_size, save_batch=False):
        ...
        obs_space_dim = len(self.observatiion_space)
    ...
    def output_results(self):
        ...
        for var, data in self.agent_stats.items():
            ...
            output_dict[var] = pd.Series(data, name=var)
