...
def save(
    hyp, nets, optimizers, buffer, episode, rewards, counters, paths=None, path=None
):
    if paths:
    ...
    if rewards:
        ...
        json_util.save(rewards, path / "rewards.json")
    if counters:
        json_util.save(dict(counters), path / "counters.json")
    json_util.save(hyp, path / "hyperparameters.json")
